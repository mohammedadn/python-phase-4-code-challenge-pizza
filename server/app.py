...
class Restaurants(Resource):
    def get(self):
        ...
        # return the list of restaurants with a 200 status cod
        return make_response(restaurants, 200)

# Adding the restaurants resource to the API at the restaurants endpoint
api.add_resource(Restaurants, "/restaurants")

#  for handling individual restaurants by ID
class RestaurantByID(Resource):
    def get(self, id):
        # Query for a restaurant by ID
        restaurant = Restaurant.query.filter_by(id=id).first()
        if restaurant is None:
            # a 404 error if the restaurant isn't found
...
# dding the RestaurantByID resource to the API at an endpoint
...
# esource for handling the creation of RestaurantPizza associations
